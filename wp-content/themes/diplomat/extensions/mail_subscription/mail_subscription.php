...
class TMM_Mail_Subscription {
    ...
    public static function init_meta_boxes() {
        add_meta_box("zipcode", esc_html__("Zip Code", 'diplomat'), array(__CLASS__, 'zipcode_field'), self::$slug, "normal", "low");
    }
    ...
    public static function save_post($post_id) {
        ...
        if (!empty($recepient_mails)){
            foreach ($recepient_mails as $email){
                ...
                $replacements['linked_post_title'] = '<a href=' . $post_url . '>' . $post_title  . '</a>';
    ...
    public static function prepare_message($str, $post){
        $replacements=array(
            ...
            '%linked_post_title%' => $post['linked_post_title'],
    ...
    public static function subscribe_request(){
        ...
        $replacements['linked_post_title'] = '';
    ...
    public static function send_email($email, $subject, $messagebody, $errors, $add_post=false){
        ...
        /* check subject */
		if (empty($subject)) {
			$subject = esc_html__("Email from subscription form", 'diplomat');
		}

		/* set message */
		$after_message = '<tr><td><br/>--------------------------------------------------------</td></tr><tr><td>' . esc_html__('This mail was sent via', 'diplomat') . ' <a href="' . site_url() . '">' . get_bloginfo('name') . '</a> ' . esc_html__('subscribtion form', 'diplomat') . '</td></tr></table>';
