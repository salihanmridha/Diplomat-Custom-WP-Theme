...
class TMM_Custom_Sidebars {
	...
	public static function get_categories_select($selected = '', $name = '', $id = '') {
		...
		$args = array(
			'show_option_none' => esc_html__('Select category', 'diplomat'),
			'hide_empty' => 0,
			'echo' => 0,
			'selected' => $selected,
			'hierarchical' => 0,
		...
		if(empty($html)){
			$html = '<select><option>'. esc_html__('No categories', 'diplomat') .'</option></select>';
		}
	...
	public static function get_pages_select($selected = '', $name = '', $id = '') {
		$args = array(
			'show_option_none' => esc_html__('Select page', 'diplomat'),
			'selected' => $selected,
			'echo' => 0,
			'name' => $name,
			'id' => $id
