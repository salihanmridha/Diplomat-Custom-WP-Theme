...
<div id="js_sidebar_panel">
	...
	<div class="option option-add-form">
		...
		<h4 class="option-title"><?php esc_html_e('Add Sidebar', 'diplomat'); ?></h4>
	...
	<div class="option">
		...
		<ul class="groups custom_sidebars_list">		
			<?php if (!empty($sidebars) AND is_array($sidebars)): ?>
				<?php foreach ($sidebars as $sidebar_id => $sidebar) : ?>
					...
					if ( $cur_lang == '' || $cur_lang === $sidebar_lang || ($sidebar_lang == '' && $cur_lang == $def_lang) ) {
						...
						<li>
							...
							<a href="#" title="<?php esc_html_e('Delete', 'diplomat'); ?>" class="remove js_remove_sidebar"
							   data-id="sidebar_<?php echo $sidebar_id ?>"></a>
							<a data-id="sidebar_<?php echo $sidebar_id; ?>" href="#"
							   title="<?php esc_html_e('Edit', 'diplomat'); ?>" class="edit js_edit_sidebar"></a>
						</li>
						<?php
					}
				...
				endforeach; ?>
			<?php else: ?>
				<li class="js_no_one_item_else"><span><?php esc_html_e('You have not created any sidebar group yet. Please create one using the form above.', 'diplomat'); ?></span></li>
			<?php endif; ?>
		</ul>
